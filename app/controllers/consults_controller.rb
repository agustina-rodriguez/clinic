class ConsultsController < ApplicationController
  ...
  def index
    ...
    # En este caso lo qeu pasa es que especialidad no está directamente relacionado con consultas, entonces:
    @consults = Consult.joins(:doctor).where(:doctor => {:speciality_id => params[:speciality_id]}) if params[:speciality_id]
    # Nos conviene hacer Consult.joins, refiriéndonos con Consult al modelo.
    @consults = @consults.where(:doctor_id => params[:doctor_id]) if params[:doctor_id]
    # @consults = Consult.joins(:doctor).where("doctors.first_name LIKE :filter OR doctors.last_name LIKE :filter", :filter => params[:filter])
